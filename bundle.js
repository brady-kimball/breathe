...
var rainbowSlice = exports.rainbowSlice = function rainbowSlice(n) {
  // 255 => '#0000FF'
  // 16711680 => '#FF0000'
  // 16777215 => '#FFFFFF'
  var step = (16711935 - 22015) / n;
  var colorArray = [];
  for (var i = 1; i < n; i++) {
...
var randomSlice = exports.randomSlice = function randomSlice(n, start) {
  // 255 => '#0000FF'
  // 16711680 => '#FF0000'
  // 16777215 => '#FFFFFF'
  var offset = start || Math.floor(Math.random() * 10000000);
  var step = (16711680 - offset) / n + 2;
  var colorArray = [];
  for (var i = 1; i <= n; i++) {
...
var helpText = exports.helpText = function helpText() {
  ...
  var $s2 = $("<span></span>").text(", a concept in math and computer science that describes a grid of cells whose values are determined by the value of their surrounding cells.");
  var $p1 = $("<p></p>").append($s1).append($cyclicLink).append($s2);

  var $s3 = $("<span></span>").text("In cyclic cellular automata, every cell can be in any one of n states.  Each cell remains in its state until a surrounding cell has a modular state 1 above it's own, at which point it copies that state.  In this implementation, the states are represented by colors, and the 'surrrounding' cells are considered the cell's ");
  var $p2 = $("<p></p>").append($s3).append($mooreLink);
  ...
  var $p4 = $("<p></p>").text("One variation is to add a 'threshold', a second constraint that means a cell will only be 'consumed' by it's surrounding cell if it has N neighbor cells with the appropriate state.  In our previous example, with a threshold of 2, means that a red cell will only become green if it has 2 green neighbor cells.");

  var $p5 = $("<p></p>").text("Breathe is a javascript implementation of this concept that explores the relationship between number of states and threshold.");
...
var Board = function () {
  ...
  _createClass(Board, [{
    ...
    value: function createGrid() {
      ...
      for (var i = 0; i < this.density; i++) {
        ...
        for (var j = 0; j < this.density; j++) {
          var randInfo = this.randColor();
          this.cells[i].push(new _cell2.default(randInfo[0], randInfo[1]));
        }
      }
    }
  }, {
    ...
    value: function randColor() {
      var id = Math.floor(Math.random() * this.COLORS.length);
      return [id, this.COLORS[id]];
    }
  }, {
    key: 'updateColors',
    value: function updateColors(cb) {
      for (var i = 0; i < this.density; i++) {
        for (var j = 0; j < this.density; j++) {
    ...
    key: 'checkColors',
    value: function checkColors(i, j) {
      var cell = this.cells[i][j];
      var neighbors = 0;
      for (var iPrime = i - 1; iPrime <= i + 1; iPrime++) {
        for (var jPrime = j - 1; jPrime <= j + 1; jPrime++) {
          if (this.inBounds(iPrime, jPrime)) {
            if (this.isSuccessor(this.cells[iPrime][jPrime], cell)) {
              neighbors += 1;
              if (neighbors >= this.threshold) {
                var id = (cell.id + 1) % this.COLORS.length;
    ...
    key: 'isSuccessor',
    value: function isSuccessor(neighbor, cell) {
...
var BoardView = function () {
  function BoardView(canvas, ctx, density, board, colors) {
  ...
  _createClass(BoardView, [{
    ...
    key: "addHelpSplash",
    value: function addHelpSplash() {
      var $helpText = Util.helpText();
    ...
    value: function bindHandlers() {
      ...
      this.brushSlider.addEventListener("change", function (e) {
        e.preventDefault();
      ...
      this.numColorsInput.addEventListener("change", function (e) {
        e.preventDefault();
    ...
    key: "setNumColors",
    value: function setNumColors(num) {
      this.numColors = num;
      $("#num-colors").val(num);
      var newColors = Util.randomSlice(num);
    ...
    key: "setThreshold",
    value: function setThreshold(num) {
    ...
    key: "setNeighborhood",
    value: function setNeighborhood(i, j, id, color) {
      for (var x = i - this.brushSize; x <= i + this.brushSize; x++) {
        for (var y = j - this.brushSize; y <= j + this.brushSize; y++) {
    ...
    key: "render",
    value: function render(defaultColor) {
      var dx = this.width / this.density;
      var x = 0;
      var y = 0;
      for (var i = 0; i < this.density; i++) {
        x = 0;
...
var _boardView = __webpack_require__(2);
...
document.addEventListener('DOMContentLoaded', function () {
  var density = 175;
  var COLORS = ["#532cf1", "#8c734e", "#c5b9ab", "#ff0008"];
  var canvas = document.getElementById('canvasEl');
  var ctx = canvas.getContext("2d");
  var board = new _board2.default(density, COLORS);
  var boardView = new _boardView2.default(canvas, ctx, density, board, COLORS);
...
var Cell = function () {
  ...
  _createClass(Cell, [{
    ...
    key: "update",
    value: function update(id, color) {
      this.nextId = id;
