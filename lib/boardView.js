...
class BoardView {
  ...
  addHelpSplash() {
    let $helpText = Util.helpText();
    let $dim = $("<section class='dim-body'></section>");
  ...
  removeSplash() {
    ...
    let fn = this.removeSplash.bind(this);
    $("body").off("click");
    $("body").off("click");
  }
  ...
  bindHandlers() {
    ...
    this.helpButton.forEach( (button) => {
        button.addEventListener("click", (e) => {
          that.removeSplash();
          e.preventDefault();
          e.stopImmediatePropagation();
    ...
    this.paintHelp.addEventListener("click", (e) => {
      that.removeSplash();
      e.preventDefault();
      e.stopImmediatePropagation();
      that.addPaintSplash();
